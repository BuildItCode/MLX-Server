...
class DropPathInput(Input):
    ...
    def _on_paste(self, event: events.Paste) -> None:
        # Textual's default Input paste REPLACES the current selection (and "select
        # all" is the common state), so a drop would silently wipe a manually typed
        # path and look like it "got unselected". Kill the selection first, then
        # check for a dropped/pasted filesystem path.
        self.selection = None
        ...
        if cleaned and os.path.isabs(cleaned) and os.path.exists(cleaned):
            event.stop()
            self.value = cleaned            # write the resolved path — survives even
            self.post_message(self.PathDropped(cleaned))  # if the screen's handler misses it
