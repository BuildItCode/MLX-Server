...
class MlxBridge:
    ...
    async def stream_chat(
        ...
        *,
        ...
        # stream_options makes the server emit a final usage chunk (prompt/completion token
        # counts) — the source of truth the TUI's context bar meters against.
            ...
            async with client.stream("POST", url, json=payload, headers=self._headers) as resp:
                ...
                async for raw in _iter_sse_lines(resp, cancel):
                    ...
                    try:
                        ...
                        continue
                    if obj.get("usage"):  # the final usage chunk (choices is empty on it)
                        usage = obj["usage"]
...
# The format engine speaks to any OpenAI-compatible server; this is the default adapter.
OpenAIEngine = MlxBridge
