...
The main model delegates to subagents automatically via deepagents' ``task``
tool, using the ``description`` to decide when to delegate."""
...
class SubagentEditorScreen(Screen):
    ...
    def compose(self) -> ComposeResult:
        ...
        with VerticalScroll(id="subagent-form"):
            ...
            yield Label("Name — the subagent's identifier")
            yield Input(id="sa-name", placeholder="Researcher")
            yield Label("Description — what this subagent does; the main model reads this to decide when to delegate")
            yield Input(id="sa-desc", placeholder="Research topics on the web and summarize findings")
    ...
    def on_mount(self) -> None:
        if self.sub:
            ...
            self.query_one("#sa-desc", Input).value = self.sub.description
    ...
    def _save(self) -> bool:
        ...
        sub.name = name
        sub.description = self.query_one("#sa-desc", Input).value.strip()
