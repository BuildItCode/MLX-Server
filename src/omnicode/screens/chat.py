...
from ..chat import capabilities, fs_tools, skills, store, voice
...
# build_openai_messages + scaled_max_tokens stay for the in-process context bar.
from ..chat.client import build_openai_messages, scaled_max_tokens
from ..models import Attachment, Chat, ChatMessage, Subagent
from ..models import ServerConfig
from ..widgets.code_block import CodeBlock
from ..widgets.path_input import DropPathInput, PathInput, resolve_path
...
class TextPromptModal(ModalScreen[Optional[str]]):
    """A tiny centered text prompt (used for naming projects, picking a working dir).

    The input is drop-aware: dragging a folder onto it (the terminal pastes the path)
    fills the field — a dropped FILE resolves to its parent directory, which is the
    natural intent for a directory prompt."""
    ...
    def compose(self) -> ComposeResult:
        with Vertical(id="modal-box"):
            ...
            yield PathInput(value=self._value, id="modal-input",
    ...
    def _dropped(self, event: DropPathInput.PathDropped) -> None:
        # A dropped file means its parent folder for a directory prompt.
        path = event.path if os.path.isdir(event.path) else os.path.dirname(event.path)
        field = self.query_one("#modal-input", Input)
        field.value = path.rstrip("/") or os.sep
        field.focus()

    def on_paste(self, event: events.Paste) -> None:
        # Fallback — same as the add-model editor's screen-level on_paste: a folder
        # dropped while focus is on the OK/Cancel button or the box (not the input)
        # bubbles up here instead of reaching DropPathInput._on_paste. Without this
        # the path is silently lost — which is why the drop "only works sometimes".
        text = event.text.splitlines()[0] if event.text else ""
        resolved = resolve_path(text)
        if not resolved or not os.path.isabs(resolved) or not os.path.exists(resolved):
            return
        event.stop()
        path = resolved if os.path.isdir(resolved) else os.path.dirname(resolved)
        field = self.query_one("#modal-input", Input)
        field.value = path.rstrip("/") or os.sep
        field.focus()
    ...
    def _ok(self) -> None:
        value = resolve_path(self.query_one("#modal-input", Input).value)
...
class SubagentsModal(ModalScreen[None]):
    """Manage subagents: create, edit, delete. Opens the SubagentEditorScreen for
    new/edit. Subagents are auto-delegated by the main model via deepagents, so there
    is no manual \"open side chat\" action here."""
    ...
    def _rebuild(self) -> None:
        ...
        for sub in self._data.subagents:
            ...
            lst.mount(Horizontal(name, edit_btn, del_btn, classes="sa-modal-row"))
...
class ChatScreen(Screen):
    ...
    def __init__(self, server_id: Optional[str] = None) -> None:
        ...
        self._gen_main = False  # is the main pane generating
        self._cancel_main = False  # Stop requested
        ...
        # Real prompt_tokens reported by the engine this session (largest across turns). The
        # char-based estimate misses tool schemas, deepagents' system prompt, and tool-result
        # replays, so when the backend reports usage it wins and the bar shows the true value.
        self._real_ctx_used: int = 0
        ...
        with Horizontal(id="chat-body"):
            with Vertical(id="chat-sidebar"):
                with Horizontal(id="sidebar-head"):
                    ...
                    yield Button("◀", id="collapse-sidebar", classes="icon-btn")
                ...
                with Horizontal(id="panes"):
                    with Vertical(id="main-pane"):
                        with Horizontal(id="chat-topbar"):
                            yield Button("▶", id="expand-sidebar", classes="icon-btn hidden")
    ...
    def _refresh_chats(self) -> None:
        ...
        lv.clear()
        chats = store.chats_in(self.data, None)
        if not chats:
    ...
    def on_unmount(self) -> None:
        ...
        # not tear the chat down.
        self._cancel_main = True
    ...
    def _update_topbar(self) -> None:
        if not self.chat:
            self.app.title = "omnicode"
            self.app.sub_title = "omnicode"
            return
    ...
    def _context_usage(self) -> Optional[tuple[int, int]]:
        ...
        if not window:
            return None
        messages = build_openai_messages(self.chat, None, self._skill_instructions())
        used = capabilities.estimate_prompt_tokens(messages)
        if self.chat.working_dir and os.path.isdir(os.path.expanduser(self.chat.working_dir)):
            used += capabilities.approx_tokens(fs_tools.system_note(os.path.expanduser(self.chat.working_dir)))
        # The backend's real prompt_tokens (when reported) already includes everything the
        # estimate can't see — tool schemas, the deepagents base prompt, tool-result replays.
        return max(used, self._real_ctx_used), window
    ...
    def _maybe_autocompact(self) -> None:
        ...
        if self._gen_main or self._compacting or not self.chat:
            return
        usage = self._context_usage()
        if not usage:
            return
    ...
    def _folder_btn(self) -> None:
        if not self.chat:
            return
        self.app.push_screen(
            TextPromptModal("Working directory", self.chat.working_dir or ""),
            self._on_folder_modal,
        )

    def _on_folder_modal(self, result: Optional[str]) -> None:
        if not self.chat or result is None:
            return
        self.chat.working_dir = result.strip() or None
        self._update_folder_btn()
        self._update_topbar()
        self._update_context_bar()
        self._persist()
    ...
    def _update_folder_btn(self) -> None:
        """Show a folder icon with the folder name next to it (when set)."""
        try:
            btn = self.query_one("#folder-btn", Button)
        except Exception:  # noqa: BLE001 — not mounted yet
            return
        if self.chat and self.chat.working_dir:
            short = self.chat.working_dir.replace(os.path.expanduser("~"), "~")
            name = os.path.basename(short.rstrip("/")) or short
            btn.label = f"▣ {name}"
            btn.set_class(True, "has-folder")
        else:
            btn.label = "▣"
            btn.set_class(False, "has-folder")
    ...
    @on(Button.Pressed, "#collapse-sidebar")
    def _collapse_sidebar(self) -> None:
        self._set_sidebar_collapsed(True)

    @on(Button.Pressed, "#expand-sidebar")
    def _expand_sidebar(self) -> None:
        self._set_sidebar_collapsed(False)

    def _set_sidebar_collapsed(self, collapsed: bool) -> None:
        """Toggle the chats sidebar: hide it to give the chat pane the full width."""
        sidebar = self.query_one("#chat-sidebar")
        sidebar.display = not collapsed
        self.query_one("#expand-sidebar", Button).set_class(not collapsed, "hidden")
    ...
    def _update_slash_menu(self) -> None:
        ...
        if not _SLASH_TRIGGER_RE.match(text):
            self._slash_items = []
            sug.display = False
            return
        prefix = text.lower()
    ...
    def _sync_send_button(self) -> None:
        """The Send/Stop button reflects whether a reply is in progress."""
        try:
            send = self.query_one("#send", Button)
            send.label = "■ Stop" if self._gen_main else "Send"
            send.variant = "error" if self._gen_main else "primary"
        except Exception:  # noqa: BLE001
            pass
    ...
    def _request_cancel(self) -> None:
        """Stop the in-flight reply: set the cancel flag and POST a cancel to the backend."""
        self._cancel_main = True
        if self._active_main is not None:
    ...
    def action_back(self) -> None:
        if self._gen_main:  # Esc stops the reply, not the screen
    ...
    def _resolve_delete(self):
        """What the delete action targets: a focused chat list item, else the open chat."""
    ...
    async def _delete_flow(self) -> None:
        ...
        if kind == "chat" and self._gen_main:
        ...
        store.save(self.data)
        self._refresh_chats()
        remaining = store.chats_in(self.data, None)
        if remaining:
    ...
    def action_edit_last(self) -> None:
        if self._gen_main or not self.chat:
            return
    ...
    def _start_compaction(self, *, auto: bool) -> None:
        ...
        if self._gen_main or self._compacting:
            if not auto:
    ...
    async def _compaction_worker(self, auto: bool) -> None:
        ...
        self._set_generating(True)  # shows Stop; Esc cancels the compact run
    ...
    async def _render_run(self, transcript, events, label: str, *, show_reasoning: bool,
        ...
        async for etype, data in events:
            if etype == "run_start":
                ...
                # Real prompt_tokens from the engine (see core/deepagents/adapter.py) — overrides
                # the char-based estimate so the context bar and auto-compact meter the true window.
                self._real_ctx_used = max(self._real_ctx_used, int(data.get("used") or 0))
                self._update_context_bar()
            elif etype == "finish":
    ...
    def _working_dir_path(self) -> Optional[str]:
        """The chat's working directory if it exists on disk, else None."""
        if self.chat and self.chat.working_dir:
            path = os.path.expanduser(self.chat.working_dir)
