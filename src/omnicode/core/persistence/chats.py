"""Persistence for chats + projects (~/.config/omnicode/chats.json)."""
...
from .config import atomic_write_text, backup_aside, config_dir
...
def load() -> ChatStoreFile:
    ...
    try:
        ...
        _backup(path)
        return ChatStoreFile()
    # Migrate: copy project working_dir + instructions into chats that reference them
    _migrate_projects_to_chats(data)
    try:
...
def _migrate_projects_to_chats(data: dict) -> None:
    """In-place: for each chat with project_id but no working_dir, copy the project's
    working_dir + instructions. Runs before model_validate so the Chat model sees the
    migrated fields directly."""
    ...
    for c in data.get("chats") or []:
        ...
        if pid and pid in projects and not c.get("working_dir"):
            p = projects[pid]
            if p.get("working_dir"):
                c["working_dir"] = p["working_dir"]
            if p.get("instructions") and not c.get("instructions"):
...
def _salvage(data) -> ChatStoreFile:
    ...
    # Migrate: if a chat has project_id but no working_dir, copy from the project
    projects = {p.id: p for p in items("projects", Project)}
    for c in chats:
        if c.project_id and not c.working_dir and c.project_id in projects:
            p = projects[c.project_id]
            c.working_dir = p.working_dir
            if p.instructions and not c.instructions:
                c.instructions = p.instructions
    return ChatStoreFile(
...
async def mutate(apply: Callable[[ChatStoreFile], Optional[ChatStoreFile]]) -> ChatStoreFile:
    """Read-modify-write the chat store under a process-wide lock: load → apply → save.

    Serializes concurrent writers (multiple frontends on the backend service) so one can't clobber
    another — replacing the fragile "re-read immediately before save" convention. ``apply`` mutates
    the loaded file in place (or returns a replacement). Returns the saved ``ChatStoreFile``."""
