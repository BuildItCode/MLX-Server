...
SYSTEM_NOTE = (
    "You have filesystem and shell tools scoped to this chat's working directory:\n  {root}\n"
    "Use ls, read_file, write_file, edit_file, glob, grep (all confined to that root) to inspect "
    "and change files, and execute to run shell commands in it.\n"
    "IMPORTANT: pass paths RELATIVE to that working directory (e.g. \"index.html\" or "
    "\"src/app.py\") — the tools are already rooted there.\n"
    "ACT on the request by CALLING these tools — when asked to create, build, fix, or change "
    "something, actually do it with write_file / edit_file / execute. Do NOT just describe "
    ...
    "open_in_browser with its RELATIVE path (e.g. \"index.html\") so the user can see it — "
    "never hand-build file:// URLs (a typo like a fused \"DesktopTest\" segment navigates "
    "the browser to a broken page).\n"
