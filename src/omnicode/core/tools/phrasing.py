...
``screens/chat.py``; re-imported there so ``omnicode.screens.chat._tool_phrase`` still resolves."""
...
def _tool_phrase(name: str, args: dict) -> str:
    ...
    # MCP tools (mcp__<server>__<tool>) → a readable "Mcp <server> <tool>".
    if name.startswith("mcp__"):
        parts = name[len("mcp__"):].split("__", 1)
        server = parts[0].replace("_", "-")
        tool = parts[1].replace("_", " ") if len(parts) > 1 else ""
        label = f"Mcp {server} {tool}".strip()
        return label[:1].upper() + label[1:]
    label = (name or "").replace("_", " ").replace("-", " ").strip() or "a tool"  # unknown
    return label[:1].upper() + label[1:]
