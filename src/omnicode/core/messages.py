...
Turns a :class:`~omnicode.models.chat.Chat` (history, mode, project + skill instructions,
...
def build_openai_messages(
    ...
    msgs: list[dict] = []
    # ``project`` is kept for back-compat but is now always None — instructions come from
    # ``chat.instructions`` directly.
    parts = [
        p.strip()
        for p in (skill_instructions, chat.instructions if not project else (project.instructions if project else ""))
        if p and p.strip()
    ]
    # The senior-engineer persona is always on (coding is no longer a toggle).
