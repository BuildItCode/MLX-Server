...
`description`) followed by a Markdown instruction body. Three origins:
  ...
  - ``custom``  — user-created, under ``~/.config/omnicode/skills``; editable.
...
from .persistence.config import config_dir
...
def _bundled_root() -> Optional[Path]:
    ...
    The skills live *inside* the package (`omnicode/skills`), so they survive
    ...
    try:
        ...
        packaged = Path(str(files("omnicode"))) / "skills"
        if packaged.is_dir():
            return packaged
