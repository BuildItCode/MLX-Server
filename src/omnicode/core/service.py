...
see :mod:`omnicode.core.server_main`) on every route except ``/healthz``, and treat a dropped
...
def _chat_summary(chat: Chat) -> dict:
    return {"id": chat.id, "title": chat.title, "model": chat.model,
            "working_dir": chat.working_dir, "updated": chat.updated}
...
async def _drive_compaction(app: Starlette, handle: RunHandle, chat: Chat) -> None:
    ...
    try:
        ...
        # summarize, not plan/review: drop plan framing for the summary request
        summary_chat = chat.model_copy(update={"mode": "build"})
        ssn = Session(chat=summary_chat, server=session.server,
    ...
    try:
        ...
        async with AsyncExitStack() as stack:
            ...
            runner = AgentRunner(
                ...
                subagents=session.subagent_specs(),
                memory_sources=session.memory_sources(),
                skill_sources=session.skill_source_paths(),
                fs_root=root,
            )
...
_SESSION_STR_FIELDS = ("project_id", "skill_id", "reasoning_effort", "mode", "title", "working_dir", "instructions", "subagent_server_id")
_SESSION_BOOL_FIELDS = ("reasoning", "web_search")
