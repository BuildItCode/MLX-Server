...
Re-export shim. The format parsers now live in :mod:`omnicode.engine.streaming`, the OpenAI
HTTP adapter in :mod:`omnicode.engine.openai`, and the OpenAI-message assembly + token
budgeting in :mod:`omnicode.core.messages` / :mod:`omnicode.core.instructions`. This
module re-exports them and keeps the :class:`ChatClient` streaming facade so existing
``omnicode.chat.client`` imports keep working."""
