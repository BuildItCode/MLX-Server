...
def backend_info_path() -> Path:
    """``$XDG_CONFIG_HOME/omnicode/backend.json`` (or ``~/.config/...``) — the discovery file the
    backend writes and the client reads. Computed here so the client stays independent of core."""
    xdg = os.environ.get("XDG_CONFIG_HOME")
    base = Path(xdg) if xdg else Path.home() / ".config"
    return base / "omnicode" / "backend.json"
...
async def spawn(*, wait: float = 20.0) -> BackendClient:
    """Start ``omnicode-backend`` as a child process and wait for it to become healthy. Uses
    ``python -m omnicode.core.server_main`` so it works without the console script on PATH."""
    proc = await asyncio.create_subprocess_exec(
        sys.executable, "-m", "omnicode.core.server_main",
